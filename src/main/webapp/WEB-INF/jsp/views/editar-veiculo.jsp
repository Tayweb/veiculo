								...
								<div class="row mb-3">
									...
									<div class="col-sm-10">
										<select class="form-select"
											aria-label="Default select example" name="marca">
											<option selected disabled="disabled">Selecione</option>
											<c:forEach items='${marcaList}' var="marca">
												<option value="${marca.id}"><c:out
														value="${marca.nome}"></c:out></option>
											</c:forEach>
										</select>
									</div>
								</div>
										...
										<div class="modal-content">
											<div class="modal-header">
												<h5 class="card-title" id="exampleModalLabel"
													style="font-size: 1.5em">Itens</h5>
														...
														<c:forEach items='${itemList}' var="item">
															<div class="form-check">
																...
																<input class="form-check-input" type="checkbox"
																	id="gridCheck1" name="itens" value="${item.id}">
																<label class="form-check-label" for="gridCheck1"><c:out
																		value="${item.nome}"></c:out></label>
															</div>
														</c:forEach>
													</div>
												</div>
							...
							<form action="filtro" method="post">
								...
								<div class="input-group mb-3">
									<input type="text" class="form-control"
										placeholder="Nome da Marca" aria-describedby="basic-addon2"
										name="nome" value="${pesquisa.nome}">
									<div class="input-group-append">
										<button class="btn btn-secondary"
											style="background-color: #012970" type="submit">
									...
									<c:forEach items='${veiculoList}' var="veiculo">
										...
										<tr>
											...
											<td><a href="${updateLink}" style="font-size: 1.2rem"><i
													...
													class="bi bi-trash"
													style="color: red; padding: 10px; font-size: 1.2rem"></i></a></td>
										</tr>
									</c:forEach>
								</tbody>
							</table>
